...
{
    public partial class reducelivestock : Form
    {
        private string accessConnString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=LoginMulti.accdb;";
        public reducelivestock()
        {
            InitializeComponent();
        }

        private void reducelivestock_Load(object sender, EventArgs e)
        {
        ...
        private void Update_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter a valid Crops ID.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (!cropsExists)
            {
                MessageBox.Show("Livestock ID does not exist or insufficient stock.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private bool CropsExists(int cropsid, int quantity)
        {
            ...
            {
                connection.Open();
                string query = "SELECT Quantity FROM Livestock WHERE LivestockID = ?";
                OleDbCommand command = new OleDbCommand(query, connection);
                command.Parameters.AddWithValue("@livestockID", cropsid);
        ...
        private void AddChangedRecordAndUpdateCrops(int cropsid, int userid, int quantity, DateTime changeddate)
        {
            using (OleDbConnection connection = new OleDbConnection(accessConnString))
            {
                ...
                try
                {
                    ...
                    string insertQuery = "INSERT INTO NotificationLivestock (ItemID, LivestockID, UserID, Quantity, Changed_Date) VALUES (?, ?, ?, ?, ?)";
                    ...
                    insertCommand.Parameters.AddWithValue("@livestockid", cropsid);
                    ...
                    string updateQuery = "UPDATE Livestock SET Quantity = Quantity - ? WHERE LivestockID = ?";
                    OleDbCommand updateCommand = new OleDbCommand(updateQuery, connection, transaction);
                    updateCommand.Parameters.AddWithValue("@Quantity", quantity);
                    updateCommand.Parameters.AddWithValue("@CropsID", cropsid);
