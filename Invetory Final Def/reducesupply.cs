...
{
    public partial class reducesupply : Form
    {
        ...
        private void reducesupply_Load(object sender, EventArgs e)
        {
        ...
        private void Update_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter a valid Supply ID.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (!cropsExists)
            {
                MessageBox.Show("Supply ID does not exist or insufficient stock.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private bool CropsExists(int cropsid, int quantity)
        {
            ...
            {
                connection.Open();
                string query = "SELECT Quantity FROM Supply WHERE SupplyID = ?";
                OleDbCommand command = new OleDbCommand(query, connection);
                command.Parameters.AddWithValue("@supplyid", cropsid);
        ...
        private void AddChangedRecordAndUpdateCrops(int cropsid, int userid, int quantity, DateTime changeddate)
        {
            using (OleDbConnection connection = new OleDbConnection(accessConnString))
            {
                ...
                try
                {
                    ...
                    string insertQuery = "INSERT INTO NotificationSupply (ItemID, SupplyID, UserID, Quantity, Changed_Date) VALUES (?, ?, ?, ?, ?)";
                    ...
                    insertCommand.Parameters.AddWithValue("@supplyid", cropsid);
                    ...
                    string updateQuery = "UPDATE Supply SET Quantity = Quantity - ? WHERE SupplyID = ?";
                    OleDbCommand updateCommand = new OleDbCommand(updateQuery, connection, transaction);
                    updateCommand.Parameters.AddWithValue("@Quantity", quantity);
                    updateCommand.Parameters.AddWithValue("@supplyid", cropsid);
